const jobValidationSchema = {
    ...
    openings: {
        ...
        custom:{
            options:function(value){
                if(value<=0){
                    throw new Error('openings should positive and greater than 0')
                }
                return true
            }
        }
        ...
        custom: {
            options: function(value) {
                if(!Array.isArray(value)) {
                    throw new Error('location should be in array')
                }
                if(value.length===0){
                    throw new Error('atleast one location is required')
                }
                return true 
            }
        }
    ...
    'experience.maxExp': {
        ...
        isNumeric: {
            options: { max: 70},
            errorMessage: 'max experience should be number'
        },
        custom:{
            options:function(value,{req}){
                if(Number(value)<Number(req.body.experience.minExp)){
                    throw new Error('minExp should less than maxExp ')
                }
                return true
            }
        }
